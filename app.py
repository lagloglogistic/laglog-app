...
def order_pdf(oid):
    ...
    doc_type = request.args.get('type','transport')
    o = next((x for x in db['orders'] if x['id']==oid), None)
    if not o: return jsonify({'error':'not found'}), 404
    if doc_type == 'frachtbrief':
        title = 'Frachtbrief' if lang=='de' else 'Bill of Lading'
        pdf = generate_order_pdf(o,cfg,lang,title_override=title)
        fname = f"Frachtbrief_{sanitize(o.get('nr',''))}.pdf"
    else:
...
def generate_order_pdf(order, cfg=None, lang='de', title_override=None):
    ...
    _waybill_title = title_override or L['waybill']
    story+=[Paragraph(_waybill_title,s['h3']),Paragraph(order.get('nr',''),s['h1']),Spacer(1,3),
