...
def generate_order_pdf(order, cfg=None, lang='de', title_override=None):
    ...
    story+=[vt,Spacer(1,8)]
    def sr(lbl,v,b,pl,name,addr,zip_,city_,rd,hf,sc,ref,zoll,anm=''):
    ...
    sd=[[Paragraph(h,s['label']) for h in [L['station'],L['times'],L['address'],L['shipping'],L['ref'],'Anmerkungen']],

        sr(f"1 · Aufnahme / Laden",order.get('s1von'),order.get('s1bis'),order.get('s1plan'),order.get('s1name'),order.get('s1addr'),order.get('s1zip'),order.get('s1city'),order.get('s1reederei'),order.get('s1hafen'),order.get('s1schiff'),order.get('s1ref'),order.get('s1zoll')),
        sr(f"2 · Gestellung / Lieferung",order.get('s2von'),order.get('s2bis'),order.get('s2plan'),order.get('s2name'),order.get('s2addr'),order.get('s2zip'),order.get('s2city'),None,order.get('s2hafen'),order.get('s2schiff'),order.get('s2ref'),None),
        sr(f"3 · Leer-Rückgabe",None,None,order.get('s3plan'),order.get('s3name'),order.get('s3addr'),order.get('s3zip'),order.get('s3city'),order.get('s3reederei'),None,None,order.get('s3ref'),order.get('s3zoll'))]
    # Station table: Station | Zeiten | Adresse | Reederei/Hafen/Schiff | Referenz | Anmerkungen
    _sw = W  # full page width
    st=Table(sd,colWidths=[28*mm, 28*mm, 52*mm, 42*mm, 28*mm, _sw-178*mm])
...
def generate_invoice_pdf(inv, order, cfg=None, lang='de'):
    ...
    # Greeting text
    _greet_de = 'Sehr geehrte Damen und Herren,<br/>vielen Dank für Ihren Auftrag und das damit verbundene Vertrauen!<br/>Hiermit stellen wir Ihnen die folgenden Leistungen in Rechnung:'
    _greet_en = 'Dear Sir or Madam,<br/>thank you for your order and the trust you have placed in us!<br/>We hereby invoice you for the following services:'
    _greet_style = ParagraphStyle('greet',fontName='Helvetica',fontSize=8,textColor=DARK,leading=12,spaceAfter=6)
